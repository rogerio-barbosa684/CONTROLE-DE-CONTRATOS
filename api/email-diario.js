...
function getSupabase() {
  ...
  if (!url || !key) {
    throw new Error('SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY devem estar configurados nas variaveis de ambiente.')
  }
...
async function getPayments() {
  const { data } = await getSupabase().from('payments').select('*').is('deleted_at', null)
  return data || []
}
  ...
  if (vencidos.length) {
    partes.push(`<div style="background:#fff3cd;border-left:4px solid #d4820a;padding:12px 16px;border-radius:0 6px 6px 0;margin-bottom:16px">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:0.85rem">
        <tr style="background:#f5f5f5"><th style="padding:6px;text-align:left">Contrato</th><th>Empresa</th><th>Fornecedor</th><th>Descricao</th><th>Vencimento</th><th>Valor</th><th>Dias</th></tr>
...
export default async function handler(req) {
  try {
    ...
    if (!cfg.email_remetente || !cfg.email_senha) {
      ...
      return new Response(JSON.stringify({ ok: true, message: 'Email not configured' }), { status: 200 })
    }
    ...
    return new Response(JSON.stringify({ ok: true, enviados, erros }), { status: 200 })
