...
def init_db():
    ...
    if not admin_exists:
        ...
        if not admin_pass_env:
            ...
            print("  Senha salva em: admin_initial_password.txt (remova apos uso)")
            print("=" * 60 + "\n")
            try:
...
def api_change_password():
    ...
    session['_password_changed_at'] = now
...
def api_users_create():
    ...
    if role not in ('admin', 'setor_admin', 'user'):
        role = 'user'
    ...
    try:
        ...
        if password:
            ...
            if uid == session.get('user_id'):
                session['_password_changed_at'] = now_str
        else:
